...
FUNCTION FluxS4(i,j)
	...
	INTEGER :: l
	DOUBLE PRECISION, DIMENSION(3) :: FluxS4, Aplus, Aminus,H_x,H_p
	...
	ELSE IF (j .EQ. Np+1) THEN
		FluxS4=hhori(Np*Nx+i)*normal(2,Np*Nx+i)*w(i+1,Np+2)*A(:,i+1,Np+2)&
...
SUBROUTINE dgdx4()
  ...
  Do j=1,Np
	 ...
	 ELSE
		...
		! On the left part of boundary, we do
			IF ( i .EQ. 1) THEN
				FluxWest=FluxW4_new(i,j)
				FluxEast=FluxW4_new(i+1,j)
			ELSE
				FluxWest=FluxEast
				FluxEast=FluxW4_new(i+1,j)
			END IF
...
FUNCTION FluxW4(i,j)
	...
	INTEGER, INTENT(in) :: i,j
	INTEGER :: l
	DOUBLE PRECISION, DIMENSION(3) :: FluxW4, Aplus, Aminus
	...
	IF (i .EQ. 1) THEN
		FluxW4=dp(i)*A(3,i,j+1)*A(:,i,j+1)
	ELSE IF (i .EQ. Nx+1) THEN
		FluxW4=dp(Nx+1)*A(3,Nx+1,j+1)*A(:,Nx+1,j+1)
	ELSE
		!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		IF (abs(aaplus-aaminus) .LT. 1E-7) THEN
			FluxW4(:)=0.d0
		ELSE
			FluxW4(:)=dp(i)*( (aaplus*Aplus(3)*Aplus(:)-aaminus*Aminus(3)*Aminus(:) &
			+ aaplus*aaminus*(Aminus(:)-Aplus(:)))/(aaplus-aaminus) )
		END IF
...
FUNCTION FluxW4_new(i,j)

	IMPLICIT NONE
	
	INTEGER, INTENT(in) :: i,j
	INTEGER :: l
	DOUBLE PRECISION, DIMENSION(3) :: FluxW4_new, Aplus, Aminus
	DOUBLE PRECISION, DIMENSION(3) :: AreconTop, AreconBot
	DOUBLE PRECISION, DIMENSION(3) :: Gradx1, Gradx2, Gradx3, Gradp1, Gradp2
	DOUBLE PRECISION, DIMENSION(3) :: Gradx4, Gradx5, Gradp4, Gradp5, Gradp3
	DOUBLE PRECISION :: aaminus, aaplus, dptt
	

	IF (i .EQ. 1) THEN
		FluxW4_new=dp(i)*A(3,i,j+1)*A(:,i,j+1)
	ELSE IF (i .EQ. Nx+1) THEN
		FluxW4_new=dp(Nx+1)*A(3,Nx+1,j+1)*A(:,Nx+1,j+1)
	ELSE
		!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		! Computation of Grad1
		AreconBot=coeff3(i-1,j  ,1)*A(:,i-1,j)+coeff3(i-1,j  ,2)*A(:,i,j  )+&
		...
		!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		! make grad4 and grad5 using coeff1
		!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		AreconBot=coeff(i-1,j-1  ,1)*A(:,i-1,j)+coeff(i-1,j-1  ,2)*A(:,i,j  )+&
		coeff(i-1,j-1  ,3)*A(:,i,j+1)+coeff(i-1,j-1  ,4)*A(:,i-1,j+1)
		AreconTop=coeff(i,j-1,1)*A(:,i,j)+coeff(i,j-1,2)*A(:,i+1,j)+&
		coeff(i,j-1,3)*A(:,i+1,j+1)+coeff(i,j-1,4)*A(:,i,j+1)
		
		Gradx4=m3(i-1,j-1,1)*(AreconTop-AreconBot)+m3(i-1,j-1,2)*(A(:,i,j+1)-A(:,i,j))
		Gradp4=m3(i-1,j-1,3)*(AreconTop-AreconBot)+m3(i-1,j-1,4)*(A(:,i,j+1)-A(:,i,j))
		
		
		AreconBot=coeff(i-1,j  ,1)*A(:,i-1,j+1)+coeff(i-1,j  ,2)*A(:,i,j+1  )+&
		coeff(i-1,j  ,3)*A(:,i,j+2)+coeff(i-1,j  ,4)*A(:,i-1,j+2)
		AreconTop=coeff(i,j,1)*A(:,i,j+1)+coeff(i,j,2)*A(:,i+1,j+1)+&
		coeff(i,j,3)*A(:,i+1,j+2)+coeff(i,j,4)*A(:,i,j+2)
		
		Gradx5=m3(i-1,j,1)*(AreconTop-AreconBot)+m3(i-1,j,2)*(A(:,i,j+2)-A(:,i,j+1))
		Gradp5=m3(i-1,j,3)*(AreconTop-AreconBot)+m3(i-1,j,4)*(A(:,i,j+2)-A(:,i,j+1))
		
		
		
		dptt=(pm2(i,j+1)-pm(i  ,j+1))
		DO l=1,3
			Aplus(l) = A(l,i  ,j+1) + minmod_new(th1*Gradx1(l)*dx/2+th2*Gradp1(l)*dptt, &
					Gradx2(l)*dx/2+Gradp2(l)*dptt,th1*Gradx3(l)*dx/2+th2*Gradp3(l)*dptt, &
					th1*Gradx4(l)*dx/2+th2*Gradp4(l)*dptt, &
					th1*Gradx5(l)*dx/2+th2*Gradp5(l)*dptt)
		END DO
		...
		AreconBot=coeff(i,j-1  ,1)*A(:,i,j)+coeff(i,j-1  ,2)*A(:,i+1,j  )+&
		coeff(i,j-1  ,3)*A(:,i+1,j+1)+coeff(i,j-1  ,4)*A(:,i,j+1)
		AreconTop=coeff(i+1,j-1,1)*A(:,i+1,j)+coeff(i+1,j-1,2)*A(:,i+2,j)+&
		coeff(i+1,j-1,3)*A(:,i+2,j+1)+coeff(i+1,j-1,4)*A(:,i+1,j+1)
		
		Gradx4=m3(i,j-1,1)*(AreconTop-AreconBot)+m3(i,j-1,2)*(A(:,i+1,j+1)-A(:,i+1,j))
		Gradp4=m3(i,j-1,3)*(AreconTop-AreconBot)+m3(i,j-1,4)*(A(:,i+1,j+1)-A(:,i+1,j))
		
		
		AreconBot=coeff(i,j  ,1)*A(:,i,j+1)+coeff(i,j  ,2)*A(:,i+1,j+1  )+&
		coeff(i,j  ,3)*A(:,i+1,j+2)+coeff(i,j  ,4)*A(:,i,j+2)
		AreconTop=coeff(i+1,j,1)*A(:,i+1,j+1)+coeff(i+1,j,2)*A(:,i+2,j+1)+&
		coeff(i+1,j,3)*A(:,i+2,j+2)+coeff(i+1,j,4)*A(:,i+1,j+2)
		
		Gradx5=m3(i,j,1)*(AreconTop-AreconBot)+m3(i,j,2)*(A(:,i+1,j+2)-A(:,i+1,j+1))
		Gradp5=m3(i,j,3)*(AreconTop-AreconBot)+m3(i,j,4)*(A(:,i+1,j+2)-A(:,i+1,j+1))
		
		
		dptt=pm2(i,j+1)-pm(i+1,j+1)
		DO l=1,3
			Aminus(l)= A(l,i+1,j+1)+minmod_new(-th1*Gradx1(l)*dx/2+th2*Gradp1(l)*dptt, &
				-Gradx2(l)*dx/2+Gradp2(l)*dptt,-th1*Gradx3(l)*dx/2+th2*Gradp3(l)*dptt, &
				-th1*Gradx4(l)*dx/2+th2*Gradp4(l)*dptt,&
				 -th1*Gradx5(l)*dx/2+th2*Gradp5(l)*dptt)
		END DO
		...
		IF (abs(aaplus-aaminus) .LT. 1E-7) THEN
			FluxW4_new(:)=0.d0
		ELSE
			FluxW4_new(:)=dp(i)*( (aaplus*Aplus(3)*Aplus(:)-aaminus*Aminus(3)*Aminus(:) &
			+ aaplus*aaminus*(Aminus(:)-Aplus(:)))/(aaplus-aaminus) )
		END IF
