...
FUNCTION FluxS2(i,j)
	...
	INTEGER :: l
	DOUBLE PRECISION, DIMENSION(3) :: FluxS2, Aplus, Aminus,H_x,H_p
	...
	ELSE IF (j .EQ. Np+1) THEN
		FluxS2=hhori(Np*Nx+i)*normal(2,Np*Nx+i)*w(i+1,Np+2)*A(:,i+1,Np+2)&
	...
	ELSE		
		...
		!FluxS2(:)= hhori((j-1)*Nx+i)*normal(1,(j-1)*Nx+i)*( (aaplus*Aplus(3)*Aplus(:)-aaminus*Aminus(3)*Aminus(:)
...
FUNCTION FluxW2(i,j)
	...
	INTEGER, INTENT(in) :: i,j
	INTEGER :: l
	DOUBLE PRECISION, DIMENSION(3) :: FluxW2, Aplus, Aminus
	...
	IF (i .EQ. 1) THEN
		FluxW2=dp(i)*A(3,i,j+1)*A(:,i,j+1)
	ELSE IF (i .EQ. Nx+1) THEN
		FluxW2=dp(Nx+1)*A(3,Nx+1,j+1)*A(:,Nx+1,j+1)
	ELSE
		!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		DO l=1,3
			Aplus(l) = A(l,i  ,j+1)+minmod(th1*Gradx1(l),Gradx2(l),th1*Gradx3(l))*dx/2.d0 &
						+minmod(th2*Gradp1(l),Gradp2(l),th2*Gradp3(l))*(pm2(i,j+1)-pm(i  ,j+1))
		END DO
		...
		DO l=1,3
			Aminus(l)= A(l,i+1,j+1)-minmod(th1*Gradx1(l),Gradx2(l),th1*Gradx3(l))*dx/2.d0 &
						+minmod(th2*Gradp1(l),Gradp2(l),th2*Gradp3(l))*(pm2(i,j+1)-pm(i+1,j+1))
		END DO
		...
		IF (abs(aaplus-aaminus) .LT. 1E-7) THEN
			FluxW2(:)=0.d0
		ELSE
			FluxW2(:)=dp(i)*( (aaplus*Aplus(3)*Aplus(:)-aaminus*Aminus(3)*Aminus(:) &
			+ aaplus*aaminus*(Aminus(:)-Aplus(:)))/(aaplus-aaminus) )
		END IF
