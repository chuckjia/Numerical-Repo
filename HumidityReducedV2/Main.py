...
while (step < numSteps):
    ...
    for i in range(1, Nx + 1):
        for j in range(1, Np + 1):
            ...
            TTemp[i][j] = T[i][j] + 0.5 * Dt * (-1 / currCell.volume *
                                             (GFlux1[i][j] - GFlux1[i][j - 1] +
                                              FFlux1[i][j] - FFlux1[i][j - 1]) +
                                             SVal1)
            qTemp[i][j] = q[i][j] + 0.5 * Dt * (-1 / currCell.volume *
                                             (GFlux2[i][j] - GFlux2[i][j - 1] +
                                              FFlux2[i][j] - FFlux2[i][j - 1]) +
                                             SVal2)
    ...
    for i in range(1, Nx + 1):
        for j in range(1, Np + 1):
            ...
            T[i][j] += Dt * (-1. / currCell.volume *
                                            (GFlux1[i][j] - GFlux1[i][j - 1] +
                                             FFlux1[i][j] - FFlux1[i][j - 1]) +
                                            SVal1)
            q[i][j] += Dt * (-1. / currCell.volume *
                                            (GFlux2[i][j] - GFlux2[i][j - 1] +
                                             FFlux2[i][j] - FFlux2[i][j - 1]) +
                                            SVal2)
...
for i in range(1, Nx + 1):
    for j in range(1, Np + 1):
        ...
        p = currCell.center.p
        vol = currCell.volume
        sum1 += (T[i][j] - TManufactured2(x, p, finalTime)) ** 2. * vol
        sum2 += vol * TManufactured2(x, p, finalTime) ** 2.
